...
def hc_theta_inf(eAM: float = 1.0, h: float = 0.0):
    """
    Coercivity & interface spin angle theta0 for soft-hard dual layer model
...
def energy_inf(
    eM: float = 1.0, eAM: float = 1.0, h: float = 0.0, ts: float = 1.0, th: float = 1.0
):
    """
    Energy for soft-hard dual layer model
    assuming that soft and hard phases are inifitely thick
    ...
    h   : Float
          Normalized external field, h = Hext / Hkhard
    ts  : Float
          Normalized thickness of the soft layer
          # ts = tsoft / dwh
    th  : Float
          Normalized thickness of the hard layer
          # th = thard / dwh
    ...
    ----------
    dwh : Float
          Critical thickness of the hard layer [unit nm]
    ...
    for i, th0 in enumerate(th0_list):
        Es = 2.0 * np.sqrt(h * eAM) * (1.0 - np.sin(th0 / 2)) - 0.5 * h * eM * ts

        Eh = (
            h * np.arccosh(np.cos(th0 / 2) / np.sqrt(h))
            - np.cos(th0 / 2) * np.sqrt(np.cos(th0 / 2) ** 2 - h)
            - h * np.arccosh(1 / np.sqrt(h))
            + np.sqrt(1 - h)
        ) + 0.5 * h * th
...
def energy_barrier_inf(eAM: float = 1.0, h: float = 0.0):
    """
    Energy barrier for soft-hard dual layer model
    assuming that soft and hard phases are inifitely thick
    ...
    for i, th0 in enumerate(th0_list):
        E[i] = (
            -2.0 * np.sqrt(h * eAM) * np.sin(th0 / 2)
            + h * np.arccosh(np.cos(th0 / 2) / np.sqrt(h))
            - np.cos(th0 / 2) * np.sqrt(np.cos(th0 / 2) ** 2 - h)
        )
...
def domain_wall_inf(
    matters: tuple[Matter, Matter] = (Matter(), Matter()),
    tsoft: float = 1.0,
    thard: float = 1.0,
    cell_size: float = 1.0,
    Hext: float = 0.0,
    ...
    """
    To generate DW configuration for soft-hard dual layer model
    assuming that soft and hard phases are inifitely thick
    ...
    tsoft  : Float
             Thickness of the soft layer [unit nm]
    thard  : Float
             Thickness of the hard layer [unit nm]
    cell_size : Float
    ...
    """
    Msoft, Asoft, Ksoft = matters[0].Ms, matters[0].Ax, matters[0].Ku
    Mhard, Ahard, Khard = matters[1].Ms, matters[1].Ax, matters[1].Ku
    ...
    for i, th0 in enumerate(th0_list):
        if th0 is not None:
            u = np.log((1.0 + np.sin(th0 / 2)) / np.cos(th0 / 2)) - x[
                x < 0
            ] / DWhard * np.sqrt(h * Msoft * Ahard / Mhard / Asoft)

            theta[i][x < 0] = 2.0 * np.arcsin(np.tanh(u))

            beta = np.sqrt(h / (1.0 - h))
            u = np.arccosh(1.0 / np.tan(th0 / 2) / beta) + x[x >= 0] / DWhard * np.sqrt(
                1.0 - h
            )
...
# =============================================================================
# Functions for soft-hard dual layer model; hard infinite but soft finite
# =============================================================================
...
def functions_for_theta(x, h, eAM, tstar):
    ...
    out = [
        2.0 * x[0] ** 2 * (1.0 - x[0] ** 2)
        + h * (1.0 - 2.0 * x[0] ** 2) * (1.0 - eAM)
        - h * (1.0 - eAM * (1.0 - 2.0 * x[1] ** 2))
    ]
    ...
    out.append(tstar - (ellipkinc(np.pi / 2, m) - ellipkinc(t0, m)) / np.sqrt(h))
...
def functions_for_hc(x, ths, eAM, phi=0.0):
    ...
    out = [
        0.5 * np.sin(x[1]) ** 2
        + x[0] * np.cos(x[1] + phi) * (1.0 - eAM)
        - x[0] * (np.cos(phi) - eAM * np.cos(ths + phi))
    ]

    out.append(
        np.sin(x[1]) * np.cos(x[1])
        - x[0] * np.sin(x[1] + phi) * (1.0 - eAM)
        - x[0] * eAM * np.sin(ths + phi) * x[2]
    )
    ...
    out.append(
        ...
        + 0.5
        * np.cos((x[1] + phi) / 2)
        / np.sqrt(np.sin((ths + phi) / 2) ** 2 - np.sin((x[1] + phi) / 2) ** 2)
        / (
            -0.5
            * np.sin((x[1] + phi) / 2)
            / np.sqrt(np.sin((ths + phi) / 2) ** 2 - np.sin((x[1] + phi) / 2) ** 2)
            ...
            * np.cos((x[1] + phi) / 2)
            * (
                (ellipeinc(np.pi / 2, m) - ellipeinc(t0, m))
                / np.sin((ths + phi) / 2)
                / np.cos((ths + phi) / 2)
                - (ellipkinc(np.pi / 2, m) - ellipkinc(t0, m))
                / np.sin((ths + phi) / 2)
                * np.cos((ths + phi) / 2)
...
def function_for_state(x, h, ths, eAM, phi=0.0):
    ...
    out = [
        0.5 * np.sin(x[0]) ** 2
        + h * np.cos(x[0] + phi) * (1.0 - eAM)
        - h * (np.cos(phi) - eAM * np.cos(ths + phi))
    ]

    return out


def hc_theta_fntsoft(eAM: float = 1.0, tstar: float = 1.0, err: float = 1.0e-6):
    """
    Coercivity & soft spin angles theta0, thetaS for soft-hard dual layer model
    ...
    print(
        "    hc: {:.5f}, theta0: {:.5f}, thetaS: {:.5f}, tstar {:.5f}\n".format(
            hc, th0c, thsc, cal_thickS(h=hc, th0=th0c, ths=thsc)
        )
    )
...
def state_thetas_fntsoft(
    eAM: float = 1.0,
    h: float = 0.0,
    tstar: float = 1.0,
    err: float = 1.0e-6,
    statec: tuple | None = None,
):
    """
    Coercivity & soft spin angles theta0, thetaS for soft-hard dual layer model
    assuming that hard phase is inifite but soft is finite
    ...
    if statec is None:
        hc, th0c, thsc, nuc_switch = hc_theta_fntsoft(eAM=eAM, tstar=tstar, err=err)
    else:
    ...
    while nloop < 10000:
        ...
        u = (1.0 - (1.0 - eAM) * np.cos(th0_try) - np.sin(th0_try) ** 2 / h / 2) / eAM
...
def energy_fntsoft(
    ...
    Elist = [None, None, None]
    th_list = state_thetas_fntsoft(eAM=eAM, h=h, tstar=tstar, statec=statec)[1:]
...
def energy_barrier_fntsoft(
    eAM: float = 1.0, h: float = 0.0, tstar: float = 1.0, statec: tuple = None
):
    """
    Energy barrier for soft-hard dual layer model
    assuming that hard phase is inifite but soft is finite
    ...
    E = [None, None]
    th_list = state_thetas_fntsoft(eAM=eAM, h=h, tstar=tstar, statec=statec)[3:]
    ...
    for i in range(2):
        ...
        if (th0 is None) or (ths is None):
            ...
            if th0 == 0.0 and ths == 0.0:
                ...
                beta = np.sqrt(h / (1.0 - h))
                u = np.arccosh(1.0 / np.tan(th0 / 2) / beta) + x[
                    x >= 0
                ] / DWhard * np.sqrt(1.0 - h)
