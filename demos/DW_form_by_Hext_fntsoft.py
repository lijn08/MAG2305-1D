# -*- coding: utf-8 -*-
"""
Created on Sat May 04 19:00:00 2024
...
def get_args_in():
    ...
    parser.add_argument('--thard',   type=float,   default=150,      help='hard layer thickness [nm], default 150')
...
#################
    ...
    ax_in = fig.add_axes([0.42, 0.22, 0.2, 0.2])
...
########################
    ...
    hc = SH.hc_theta_fntsoft(eAM = eAM, tstar = tstar)[0]
    Hc = hc * Hkhard

    # stable state
    if -args.Hext <= Hc:
        pinned = True
        y0 = SH.domain_wall_fntsoft(matters=matters, 
                                    tsoft=args.tsoft, thard=args.thard,
                                    cell_size=args.cell, Hext=args.Hext
                                    )[0]
        E0 = SH.energy_fntsoft( eM = args.Msoft/args.Mhard,
                               eAM = eAM,
                                 h = args.Hext/Hkhard,
                                ts = args.tsoft/dhard,
                                th = args.thard/dhard
...
###############
    ...
    for n in range(args.niters):
        ...
        if n % args.nplot == 0 or n == args.niters-1:
            ...
            ax.text(x=0.8*xlim0, y=0.1, s="soft\nphase",
