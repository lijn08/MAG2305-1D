...
def get_args_in():
    ...
    parser.add_argument(
        ...
        type=float,
        default=150,
        help="hard layer thickness [nm], default 150",
    )
    parser.add_argument(
        "--Mhard",
        type=float,
...
def main(args, path):
    fig, ax, axi = plot.create_canvas(axip=0.42)
    ...
    x = (
        np.linspace(
            start=0.0, stop=cell_count * args.cell, num=cell_count, endpoint=False
        )
    ...
    if -args.Hext <= Hc:  # pinned
        ...
        y1 = None
        E0 = (
            (args.Hext * args.Msoft * cell_soft + args.Hext * args.Mhard * cell_hard)
            * args.cell
            * 1.0e-7
        )
