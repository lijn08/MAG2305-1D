...
**Micromagnetic simulation** is a powerful methodology to study the hysteresis phenomenon and microscopic magnetic configurations. Good performance for a micromagnetic simulation requires fineness of simulation settings (such as the discretization of unit cell and the length of time step), sometimes yielding heavy models and prolonged time costs
...
##### The features of simulation samples created in `MAG2305-1Dlayers`
...
2. The energy of consideration reads:
    $$\mathscr{E} = \sum_{i} \{ K_u^i\sin^2\theta_i - M_s^i H_{ext}\cos\theta_i + A_x^i (\frac{\theta_{i+1} - \theta_{i-1}}{2\Delta})^2 \}$$
...
##### Examples of what can be done by `MAG2305-1Dlayers`
...
- `matters` *tuple of Matter* : to define magnetic parameters for each matter sequentially, containing the following values:
...
Thereafter, spin evolution can be simulated via method `SpinDescent`:
...
One can plot the stable state for visualization. (Make sure `matplotlib` is installed).
...
<div align="center">
FIG.2 Pinned domain wall as stable state
</div>
<br>
